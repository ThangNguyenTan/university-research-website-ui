...
import { updatePeople, getPeople, fetchJobs, fetchRoles } from '../../../apis';
...
function PeopleUpdate() {
  ...
  const mutation = useMutation({
    mutationFn: (person) => updatePeople(selectedName, person),
  });

  const handleDisplayMain = () => {
    const { isLoading, isError, error } = getPeopleQuery;
      ...
      return (
        <Container>
          <Row className="justify-content-center">
            <Col lg="8">
              <h1 className="page-title">Something went wrong while fetching people</h1>
    ...
    const person = _.get(getPeopleQuery, 'data', null);
    ...
    const { id: personId, name, email, description } = person;
    const personRoleId = _.get(person, 'role._id', '');
    const personJobId = _.get(person, 'job._id', '');

    return (
      <Container>
        <Row className="justify-content-center">
          <Col lg="8">
            <h1 className="page-title">Update People</h1>
            <Formik
              enableReinitialize
              initialValues={{
                name,
              ...
              onSubmit={(values, { setSubmitting, resetForm }) => {
                setSubmitting(true);
                mutation.mutate(
                  { ...values, coverImageFile, id: personId },
                  {
                    onSuccess: () => {
                      resetForm();
                      createToast('Successfully updated people', 'success');
                      setSubmitting(false);
                      navigate('/manage/people');
                    },
                    onError: () => {
                      createToast('Failed to update people', 'error');
                      setSubmitting(false);
                    },
                  }
                );
